 ...
 * Maneja la lógica de negocio y la interacción entre la vista (Clientes.java)
 * y el modelo de datos (ClienteDAO).
 */
public class ClienteControlador {
     ...
     * @param vista La instancia del formulario Clientes que este controlador manejará.
     */
    ...
    /**
     * Obtiene todos los clientes de la base de datos y los muestra en la JTable.
     */
    private void cargarTodosLosClientes() {
        try {
            // 1. Pide al DAO la lista completa de clientes
            List<ClienteDTO> clientes = dao.listar();
    ...
    /**
     * Recoge los datos del formulario, los valida y los guarda en la base de datos.
     * Después de guardar, refresca la tabla.
     */
    ...
    private void buscarCliente() {
        ...
        if (codigoBuscado != null && !codigoBuscado.trim().isEmpty()) {
            try {
                // 3. Llamamos al DAO
                Optional<ClienteDTO> clienteOpt = dao.buscarPorCodigo(codigoBuscado.trim());
    ...
    private void cargarClientesFrecuentes() {
        try {
            ...
            int topLimit = 5; 
            List<ClienteFrecuenteDTO> clientes = dao.listarFrecuentes(topLimit);
    ...
    private void cargarHistorialCompras(String codCli) {
        ...
        try {
            List<HistorialCompraDTO> historial = dao.listarHistorialCompras(codCli);
            for (HistorialCompraDTO compra : historial) {
                Object[] row = {
                    compra.codProducto(),
                    compra.descripcion(),
                    compra.fechaCompra(),
